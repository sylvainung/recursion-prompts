...
var sumBelow = function(n) {
  ...
  return (intType * (result += sumBelow(result)));
};
...
var range = function(x, y) {
  ...
  if ((x < y && nextNumber < y) || (x > y && nextNumber > y)) {
    result.push(nextNumber);
  }

  return result.concat(range(nextNumber, y))
};
...
var rMap = function(array, callback) {
  ...
  var current = arrayCopy.shift();
  result.push(callback(current));

  return result.concat(rMap(arrayCopy, callback));
};
